...
export function bundle(options: Options): BundleResult {
    ...
    const excludeLines = optValue(options.excludeLines, []);
    ...
    content += usedTypings.filter((parse: Result) => {
        ...
        if (inSourceTypings(parse.file)) {
            return formatModule(parse.file, parse.lines.filter(line => {
                let keep = true;
                for (let len = excludeLines.length; --len >=0;) {
